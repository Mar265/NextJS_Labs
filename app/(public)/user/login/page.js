...
function LoginForm() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!user.emailVerified) {
        setVerificationMessage('Twój e-mail nie został zweryfikowany.');
        setShowResendVerification(true); // Pokaż opcję wysłania e-maila weryfikacyjnego
        return;
      }
  ...
  const resendVerificationEmail = async () => {
    try {
      ...
      if (!email) {
        setVerificationMessage('Podaj adres e-mail, aby wysłać weryfikację.');
        return;
      }
      ...
      if (user.emailVerified) {
        setVerificationMessage('Twój e-mail został już zweryfikowany.');
        setShowResendVerification(false);
        return;
      }

      await sendEmailVerification(user);
      setVerificationMessage('E-mail weryfikacyjny został ponownie wysłany. Sprawdź swoją skrzynkę pocztową.');
    } catch (error) {
      console.error('Błąd wysyłania e-maila weryfikacyjnego:', error);
      setVerificationMessage('Wystąpił problem z wysyłaniem e-maila weryfikacyjnego. Spróbuj ponownie.');
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-var(--primary-dark) to-var(--primary)">
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {verificationMessage && (
            <div className="text-orange-500 text-sm font-medium">{verificationMessage}</div>
          )}
          <div>
            <label
              htmlFor="email"
