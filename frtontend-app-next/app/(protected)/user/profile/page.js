...
const ProfilePage = () => {
  const { user: contextUser, loading } = useAuth(); // Użytkownik z kontekstu
  const [firebaseUser, setFirebaseUser] = useState(null); // Użytkownik bezpośrednio z Firebase
  const [username, setUsername] = useState('');
  const [photoURL, setPhotoURL] = useState('');
  const [successMessage, setSuccessMessage] = useState('');
  const [errorMessage, setErrorMessage] = useState('');
  ...
  useEffect(() => {
    const auth = getAuth();
    const currentUser = auth.currentUser; // Pobierz użytkownika bezpośrednio z Firebase
    if (currentUser) {
